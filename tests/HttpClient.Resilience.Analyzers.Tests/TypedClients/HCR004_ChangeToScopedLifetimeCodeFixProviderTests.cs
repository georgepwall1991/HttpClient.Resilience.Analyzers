...
public sealed class HCR004_ChangeToScopedLifetimeCodeFixProviderTests
{
    ...
    [Fact]
    public async Task CodeFix_ChangesTypeOfOverloadToScoped()
    {
        const string source = """
            public static class Registrations
            {
                ...
                {
                    services.AddHttpClient<PaymentsClient>();
                    services.AddSingleton(typeof(PaymentJob));
                }
            }
            ...
            {
                public static IServiceCollection AddHttpClient<TClient>(this IServiceCollection services) => services;
                public static IServiceCollection AddSingleton(this IServiceCollection services, System.Type serviceType) => services;
        ...
        Assert.Contains("services.AddScoped(typeof(PaymentJob));", fixedSource, System.StringComparison.Ordinal);
    ...
    public async Task CodeFix_FixAllChangesEveryFlaggedSingletonRegistration()
    {
        ...
        var diagnosticsBefore = await AnalyzerVerifier<HCR004_TypedClientInjectedIntoSingletonAnalyzer>.GetDiagnosticsAsync(source);
