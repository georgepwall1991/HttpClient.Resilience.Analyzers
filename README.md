...
- `HCR005` for duplicate typed-client service registrations, including visible `IServiceCollection` and minimal-hosting `Services` receiver validation, namespace-aware qualified registration matching, `typeof(...)` standalone registrations, visible factory registrations that construct the typed-client implementation, and a code fix.
