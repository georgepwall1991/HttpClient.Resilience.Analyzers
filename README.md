...
- `HCR002` for static or singleton-owned manual `HttpClient` field/property initializers and assignments without `PooledConnectionLifetime`, with resolved custom client types skipped, configured-handler and namespace-aware qualified-registration recognition, plus a safe code fix for parameterless field initializers.
