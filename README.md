...
- `HCR003` for `IHttpClientFactory` clients cached through assignments or initializers into static fields/properties or known singleton fields/properties, including namespace-aware factory receiver validation, qualified singleton registrations, and visible singleton factory registrations that construct an implementation.
...
- `HCR005` for duplicate typed-client service registrations, including visible `IServiceCollection` receiver validation, namespace-aware qualified registration matching, `typeof(...)` standalone and factory registrations, and a code fix.
