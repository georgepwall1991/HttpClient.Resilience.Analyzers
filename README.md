...
- `HCR002` for static or singleton-owned manual `HttpClient` field/property initializers and assignments without `PooledConnectionLifetime`, with resolved custom client types skipped, configured handler local/field/property, namespace-aware qualified-registration recognition, visible singleton factory registrations that construct implementations, plus a safe code fix for parameterless field initializers.
