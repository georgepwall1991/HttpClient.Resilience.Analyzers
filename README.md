...
- `HCR080` for obvious unbounded `Task.WhenAll` HTTP fan-out, with BCL `Task` and resolved `HttpClient` receiver validation plus resolved or visibly declared same-receiver `SemaphoreSlim` gating, custom-client, and local/member connection-limit exclusions including `this.`-qualified members and shared handler fields.
