...
| `HCR002` | Yes | Yes | Static or singleton-owned manual client initializers and assignments without `PooledConnectionLifetime`, including resolved custom client filtering, recognition for configured handler fields, and namespace-aware qualified singleton registrations; code fix is limited to safe parameterless client initializers. |
