...
Reports duplicate `AddStandardResilienceHandler()` calls and duplicate same-name `AddResilienceHandler("name", ...)` calls in the same fluent registration chain when the chain visibly comes from `AddHttpClient(...)` or an `IHttpClientBuilder` receiver. Qualified builder type names are matched against `Microsoft.Extensions.DependencyInjection.IHttpClientBuilder`, different custom resilience handler names are allowed, and lookalike methods on unrelated custom builders are skipped.
