...
This catches traditional `Startup` methods, minimal hosting configuration, extension methods, registrations split across files, visible qualified type names, one- or two-generic-argument registration forms, and `typeof(...)` service registrations such as `AddTransient(typeof(PaymentsClient))` or `AddTransient(typeof(PaymentsClient), sp => ...)`. Registration receivers are validated as visible `IServiceCollection` shapes when their declaration is available, and explicitly qualified registration type names are compared namespace-sensitively, so same-named typed clients in different namespaces are skipped.
