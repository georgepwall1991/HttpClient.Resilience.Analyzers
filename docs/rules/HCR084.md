...
The implementation reports `IHttpClientFactory.CreateClient("name")` string literals, including values reached through visible unreassigned local initializers, when the same string literal is used in a visible `AddHttpClient("name")` registration in the compilation.
