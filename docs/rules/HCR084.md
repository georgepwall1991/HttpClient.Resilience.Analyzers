...
The implementation reports `IHttpClientFactory.CreateClient("name")` string literals when the same name is used in a visible `AddHttpClient("name")` registration in the compilation. Both registration and usage names may be reached through visible unreassigned local initializers.

It validates `Microsoft.Extensions.DependencyInjection.IServiceCollection` and `System.Net.Http.IHttpClientFactory` receivers when semantic information is available, requires resolved registrations to return `IHttpClientBuilder`, skips shared constants, reassigned or otherwise unresolved local names, and other non-literal name expressions, and skips resolved custom registration or factory lookalikes, including same-named types declared in other namespaces and custom `CreateClient` extension overloads.
