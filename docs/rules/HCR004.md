...
A code fix rewrites the flagged `AddSingleton` registration into `AddScoped`, preserving type arguments and surrounding trivia, so the consuming service shares the scoped lifetime of the typed client it resolves. Fix All renames every flagged singleton registration in the document at once. The fix is only offered when the diagnostic points at a visible `AddSingleton` call; factory-heavy or dynamically composed registrations should be reviewed manually.
