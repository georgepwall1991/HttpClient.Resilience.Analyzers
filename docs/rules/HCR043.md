...
When `AddRetry` is passed `new HttpRetryStrategyOptions { ... }`, the code fix introduces a local, calls `DisableForUnsafeHttpMethods()`, and passes that local to `AddRetry`. Expression lambdas are converted to blocks. When `AddRetry` already receives an `HttpRetryStrategyOptions` local, the fix inserts the `DisableForUnsafeHttpMethods()` call on that variable instead of introducing a duplicate. The fix is withheld when the argument is any other expression (including generic `RetryStrategyOptions<T>`), so an existing retry policy is never overwritten.
