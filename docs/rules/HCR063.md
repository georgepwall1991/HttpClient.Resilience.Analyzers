...
The code fix replaces `.Result` or `.GetAwaiter().GetResult()` with `await` when the nearest containing method, local function, or lambda is already `async`. It preserves member-access precedence with parentheses when needed. The fix is not offered for synchronous functions or `.Wait()` because those cases can require signature and control-flow changes.
