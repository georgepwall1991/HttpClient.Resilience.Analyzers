...
The implementation reports assignments or field initializers from visible `IHttpClientFactory.CreateClient(...)` calls into static fields, into instance fields on types whose names end with `Singleton`, and into instance fields on services known from compilation-wide `IServiceCollection`-shaped `AddSingleton<TService>()` or `AddSingleton<TService, TImplementation>()` registrations. Qualified singleton registrations are matched against the declared containing type namespace so same-named services in other namespaces are skipped, and lookalike `CreateClient(...)` methods on other factory types are skipped.
