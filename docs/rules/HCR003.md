...
The implementation reports assignments or field initializers from `.CreateClient(...)` into static fields, into instance fields on types whose names end with `Singleton`, and into instance fields on services known from compilation-wide `IServiceCollection`-shaped `AddSingleton<TService>()` or `AddSingleton<TService, TImplementation>()` registrations. Known singleton matching compares visible simple and qualified registration type names.
