---
title: HCR041 — AddStandardResilienceHandler retrying unsafe HTTP methods
description: Warns when standard HTTP resilience retries POST, PUT, PATCH, DELETE, or CONNECT unless DisableForUnsafeHttpMethods is configured.
---
...
When the handler already has a single-lambda configure argument, the code fix instead injects `options.Retry.DisableForUnsafeHttpMethods();` as the first statement of that lambda, preserving the existing configuration. The injection is withheld when the lambda already calls `DisableForUnsafeHttpMethods` or assigns `ShouldHandle` on the retry options — including calls that bind to custom extensions — so an existing retry policy is never overwritten; other shapes should be reviewed and composed manually.
