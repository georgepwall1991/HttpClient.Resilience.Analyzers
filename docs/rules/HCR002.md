...
The implementation reports static `HttpClient` fields/properties and instance `HttpClient` fields/properties on known singleton services when they are initialized or assigned with a default handler. Resolved custom types named `HttpClient` or `SocketsHttpHandler` are skipped. Qualified singleton registrations are matched against the declared containing type namespace so same-named services in other namespaces are skipped. It does not report ordinary instance members, clients initialized or assigned with a `SocketsHttpHandler` object initializer that sets `PooledConnectionLifetime`, or clients initialized from a visible local or field handler whose initializer sets `PooledConnectionLifetime`.
