---
title: HCR002 — Configure PooledConnectionLifetime on long-lived HttpClient
description: Warns when a static or singleton HttpClient is missing SocketsHttpHandler.PooledConnectionLifetime and can serve stale DNS.
---
...
For simple parameterless field or static property `HttpClient` initializers, the code fix replaces the default initializer with fully qualified framework `HttpClient` and `SocketsHttpHandler` types configured with `PooledConnectionLifetime = global::System.TimeSpan.FromMinutes(2)`. Fully qualifying the generated types avoids user-defined `SocketsHttpHandler` or `TimeSpan` names shadowing the framework APIs. When the initializer passes a single inline `SocketsHttpHandler` creation, the fix configures that handler in place instead, adding or appending the `PooledConnectionLifetime` assignment after verifying the framework type. An explicit `disposeHandler` argument is preserved untouched. The fix is withheld when the handler comes from another variable, the inline handler takes constructor arguments, or the creation contains comments or preprocessor directives, because preserving handler configuration and annotations requires manual review.
