...
A code fix adds an explicit client name derived from the implementation type in kebab-case (for example `StripePaymentsClient` becomes `"stripe-payments-client"`), so conflicting registrations stop sharing the implicit name. Fix All names every flagged registration at once, and each registration receives a distinct name because it is derived from its own implementation type. The fix is only offered when the diagnostic points at a visible `AddHttpClient<TService, TImplementation>()` call without a name argument; intentional sharing of one explicitly named client should be written by hand.
