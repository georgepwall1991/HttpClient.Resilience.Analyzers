...
The code fix inserts `response.EnsureSuccessStatusCode()` immediately after a single response-local declaration. This placement covers both straight-line and nested content reads while ensuring failure is observed before any subsequent response processing.
