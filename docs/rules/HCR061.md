...
The code fix inserts `response.EnsureSuccessStatusCode()` immediately before a content-read statement in the same block. It is intentionally not offered when the read is nested in control flow, where the correct failure behavior and placement require a developer decision.
