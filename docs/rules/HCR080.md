...
It does not report when the lambda visibly gates work with `SemaphoreSlim.WaitAsync(...)` and `Release()`, or when code uses a bounded `Parallel.ForEachAsync` shape instead of `Task.WhenAll`.
