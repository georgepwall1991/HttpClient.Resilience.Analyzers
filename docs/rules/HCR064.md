# HCR064

Use cancellation-aware HTTP APIs when a token is available.

## Why

Outbound HTTP calls that omit an available `CancellationToken` can keep sockets, retries, streams, and request work alive after the caller has already timed out or disconnected. Passing the token lets the call stop promptly and keeps cancellation behavior consistent through the outbound path.

## Bad

```csharp
public Task<HttpResponseMessage> GetAsync(
    HttpClient client,
    CancellationToken cancellationToken)
{
    return client.GetAsync("https://example.com");
}
```

```csharp
public Task<string> ReadAsync(
    HttpResponseMessage response,
    CancellationToken cancellationToken)
{
    return response.Content.ReadAsStringAsync();
}
```

## Better

```csharp
public Task<HttpResponseMessage> GetAsync(
    HttpClient client,
    CancellationToken cancellationToken)
{
    return client.GetAsync("https://example.com", cancellationToken);
}
```

```csharp
public Task<string> ReadAsync(
    HttpResponseMessage response,
    CancellationToken cancellationToken)
{
    return response.Content.ReadAsStringAsync(cancellationToken);
}
```

## Current Detection

The implementation reports visible `HttpClient` async calls such as `GetAsync(...)`, `PostAsync(...)`, `SendAsync(...)`, and common `HttpContent` read calls such as `ReadAsStringAsync()` when a `CancellationToken` parameter or prior local is visible in the containing method, lambda, or block and the invocation does not pass a token argument.

Receivers are validated with Roslyn type information when available, with syntactic fallback for unresolved snippets that visibly declare an `HttpClient` receiver. Calls that already pass a `CancellationToken`, calls without any visible token in scope, APIs without a token overload, and resolved custom `HttpClient` lookalikes are skipped.

The code fix passes the available token as a named `cancellationToken` argument when exactly one local or parameter is in scope and the analyzer has found a cancellation-aware overload. It is intentionally not offered when multiple tokens would make the correct cancellation boundary ambiguous.

## Suppression

Suppress only when the operation must intentionally outlive the caller's cancellation boundary or when a token is managed by a surrounding framework layer that is not visible to the analyzer.

## References

- [Cancellation in managed threads](https://learn.microsoft.com/en-us/dotnet/standard/threading/cancellation-in-managed-threads)
- [HttpClient guidelines for .NET](https://learn.microsoft.com/en-us/dotnet/fundamentals/networking/http/httpclient-guidelines)
- [HttpClient.GetAsync API](https://learn.microsoft.com/en-us/dotnet/api/system.net.http.httpclient.getasync)
