...
When the nearest containing local function or method, or the class primary constructor, already has an `IHttpClientFactory` parameter, the code fix replaces `new HttpClient()` with `<factoryParameter>.CreateClient()`. This nearest-scope selection keeps a local function's factory parameter from accidentally being replaced with an outer method parameter. When no parameter is in scope, the fix reuses a readable, non-nullable `IHttpClientFactory`-typed field or property declared on the containing type — preferring members whose name contains `Factory`, otherwise ordering by name — and skips members shadowed by an in-scope local or parameter. The fix is not offered in static contexts and does not add constructor parameters or change DI registrations automatically.
