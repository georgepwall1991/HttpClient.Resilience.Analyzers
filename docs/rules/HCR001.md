...
The implementation reports high-confidence `new HttpClient()` usage when it appears in likely request-path types, inside loops, or in `using` ownership patterns, including top-level statement loops and `using var` declarations. Resolved custom types named `HttpClient` are skipped. Test types ending with `Test` or `Tests`, and common attributed test contexts such as `[Fact]`, `[Theory]`, `[Test]`, `[TestMethod]`, and `[TestClass]`, are ignored.
