...
It skips registrations that visibly set `BaseAddress` in the `AddHttpClient(...)` configuration delegate or a chained `ConfigureHttpClient(...)` delegate. It also skips absolute HTTP/HTTPS URLs, unregistered client classes, and resolved custom `HttpClient` lookalikes.
