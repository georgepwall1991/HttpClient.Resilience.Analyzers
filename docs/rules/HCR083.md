...
It skips registrations that visibly set `BaseAddress` in the `AddHttpClient(...)` configuration delegate or a chained `ConfigureHttpClient(...)` delegate. It also skips absolute HTTP/HTTPS URLs, unregistered client classes, resolved custom `HttpClient` lookalikes, and custom extensions that reuse HTTP method names on a real `HttpClient`.
