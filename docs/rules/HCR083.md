...
The implementation reports registered typed clients from `AddHttpClient<TClient>()` and `AddHttpClient<TService, TImplementation>()` when their implementation type uses a relative string URL in common `HttpClient` calls such as `GetAsync(...)`, `PostAsync(...)`, `PutAsync(...)`, `PatchAsync(...)`, `DeleteAsync(...)`, `GetStringAsync(...)`, `GetStreamAsync(...)`, or `GetByteArrayAsync(...)`. It also checks relative request URIs in inline or visible local `HttpRequestMessage` instances passed to `HttpClient.Send(...)` or `SendAsync(...)`, including split declarations and assignments when no later reassignment obscures the value.
