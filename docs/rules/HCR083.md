...
It skips registrations that visibly set `BaseAddress` in the `AddHttpClient(...)` configuration delegate or a framework-owned chained `ConfigureHttpClient(...)` delegate. Resolved custom `ConfigureHttpClient` extensions are not treated as configuration evidence. It also skips absolute HTTP/HTTPS URLs, unregistered client classes, resolved custom `HttpClient` lookalikes, and custom extensions that reuse HTTP method names on a real `HttpClient`.
