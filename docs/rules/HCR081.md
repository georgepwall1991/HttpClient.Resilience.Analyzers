...
It recognizes `using var`, `using (stream)`, direct `Dispose()` and `DisposeAsync()`, disposal in `finally`, returning the stream, and returning an object that is visibly constructed or initialized with the stream. Disposal, `using` ownership, direct returns, and returned-owner construction can flow through visible local alias chains populated by initializers or top-level standalone assignments. Locals, aliases, and returned owner locals are reassignment-aware, resolved custom `HttpClient` lookalikes are skipped, and resolved stream methods must belong to the BCL `HttpClient` or `HttpContent` types rather than unrelated custom extensions that reuse the same names.
