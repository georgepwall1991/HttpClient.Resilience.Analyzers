...
It recognizes `using var`, `using (stream)`, direct `Dispose()` and `DisposeAsync()`, disposal in `finally`, returning the stream, and returning an object that is visibly constructed or initialized with the stream. Disposal and `using` ownership can flow through visible, unreassigned local alias chains. Locals and aliases are reassignment-aware, resolved custom `HttpClient` lookalikes are skipped, and resolved stream methods must belong to the BCL `HttpClient` or `HttpContent` types rather than unrelated custom extensions that reuse the same names.
